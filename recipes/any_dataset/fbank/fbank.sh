...
if [ ! -h "utils" ]; then
    echo "utils does not exist, creating sybolic link"
    ln -s ../../wsj/s5/utils .
fi

if [ ! -h "steps" ]; then
    echo "steps does not exist, creating sybolic link"
    ln -s ../../wsj/s5/steps .
fi

if [ ! -h "sid" ]; then
    echo "sid does not exist, creating sybolic link"
    ln -s ../../sre08/v1/sid .
fi

if [ ! -d "local" ]; then
    echo "local does not exist, creating sybolic link"
    mkdir local
	ln -s ../../../voxceleb/v1/local/nnet3 local/
fi
...
if $data_aug; then
    ...
    utils/combine_data.sh data/${data_folder}_aug data/${data_folder}_reverb data/${data_folder}_noise data/${data_folder}_music data/${data_folder}_babble

    # Make Fbank for the augmented data.  Note that we do not compute a new
    # vad.scp file here.  Instead, we use the vad.scp from the clean version of
    # the list.
    steps/make_fbank.sh --fbank-config $fbank_config --nj $nj --cmd "run.pl" data/${data_folder}_aug exp/make_fbank `pwd`/fbank

    # Combine the clean and augmented VoxCeleb2 list.  This is now roughly
    # double the size of the original clean list.
    utils/combine_data.sh data/${data_folder}_all data/${data_folder}_aug data/${data_folder}
	data_folder=${data_folder}_all
fi
